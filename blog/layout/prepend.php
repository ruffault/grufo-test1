...
//echo $mode.' - '.$year.' - '.$month.' - '.$day.' - '.$post_id.' - '.$cat_id; exit;
...
# Activation du cache SQL si possible
if (defined('DC_CACHE_DIR') && $mode != 'search' && !isset($_POST['c_nom'])) {
	$con->setCache(DC_CACHE_DIR,filemtime(DC_UPDATE_FILE));
}
			...
			else
			{
				...
				else
				{
					...
					&& $news->f('user_email') != $c_mail)
					{
						...
						"X-Mailer: DotClear\r\n".
						'X-Blog: http://'.$_SERVER['SERVER_NAME'].dc_blog_url;
