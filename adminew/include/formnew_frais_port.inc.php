...
<nav>
<form  method="post" action="index2.php?page=new_frais_port"> <!--debut du formulaire pour choisir le pays --> 
	<p>
...
?>
			...
			</p> <!-- fin d'un bloc editition mode  FP_edit_mode-->
<?php 				} ?>
		<br><p class="FP_lib_mode"> Mode de Livraison: <strong><?php echo $regle['mode'];?> </strong></p>
<?php	 		}?>
	
		<!--on écrit maintenant pour chaque poids son prix plus les icones de modification-->
		<p class="FP_regles" style="display: inline">
		<span class='FP_poids'> <strong> <= </strong> <?php echo $regle['poids_tarif'];?> </span>
		<span class='FP_prix'>Prix: <?php echo $regle['prix'] ; ?> </span>
...
<section id="FP_edit"><!-- début du bloc mode de calcul FP_edit -->
<?php 
...
<?php 		if ((isset($_GET['type']) ? $_GET['type'] : "") <> "dup") //cas standard sans duplication
		...
		<label for= "pays">Pays: <?php echo abrev2lang($pays); ?> </label>
			<input type="hidden" name="pays" id="pays" value="<?php echo $pays; ?>" readonly>
<?php 		}
...
<?php 			} ?>
	...
	<?php } // fin du champ pour pays export ?>
		...
		<input type="text" name="mode_calcul" id="mode_calcul" value="<?php echo (((isset($_GET['type']) ? $_GET['type'] : '') == "dup") ? $_GET['mode_calc'] : $mode_calcul[$row['mode_calcul']]['nom']); ?>"></th>
		</tr>		
		<tr>
		...
		<tr>
			<th><strong>Limite poids</strong></th>
