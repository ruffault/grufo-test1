...
function reject_order($ref) {
 //Réception de la commande rejeter
	...
	$sql_ref_order_result = mysqli_query($link,$sql_ref_order);
	$nb = mysqli_num_rows($sql_ref_order_result);
	$val_order = mysqli_fetch_array($sql_ref_order_result);
...
$correspondence = all_correspondence($val_order['id_commande']);	
  ...
  $sql_panier = "SELECT panier.id_commande, panier.id_produit, panier.quantite, produit.reference, produit.id_produit, produit.nom_fr
                 ...
                 AND commande.id_commande = '".$val_order['id_commande']."'
                 ;";
  $sql_panier_result = mysqli_query($link,$sql_panier);
  //$val_panier = mysqli_fetch_array($sql_panier_result);	
...
// detail de commande    	
	...
	while ($data = mysqli_fetch_object($sql_panier_result))
    {
    	echo '<tr>';
    	echo '<td>&nbsp;</td>';
...
function reject_order_form() {
  echo '
	<fieldset>
  	<p>Veuillez saisir la référence de la commande rejetée :</p>
