...
$sql_users = "SELECT *
              ...
              LIMIT 1
							;";
$res = mysqli_query($link,$sql_users);
...
echo '<img src="css/flag_' . $val['aplan'] . '.gif" title="Parle ' . convert_lang((isset($val_livraison['aplan']) ? $val_livraison['aplan'] : '') ) . '" alt="" /> ';
...
mysqli_query($link,$sql_showpanier);
...
if (mysqli_num_rows($sql_archive_result) != 0)
{
	echo '<h3>Commande en traitement</h3>';
	echo '<table id="users">';
	echo '<tr>';
	echo '<th>&nbsp;</th>';
	if ((isset($_GET['tri']) ? $_GET['tri'] : '') == "ref" && (isset($_GET['invert']) ? $_GET['invert'] : '') == 1)
		echo '<th>Réf</th>';
	else
		echo '<th>Réf</th>';
