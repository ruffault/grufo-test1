...
$sql_viewcommande_result = mysqli_query($link,$sql_viewcommande);
$total_cmd = mysqli_num_rows($sql_viewcommande_result);
...
echo '<th>Nom</th>';
echo '<th>Prix HT</th>';
if (!isset($_GET["traite"] ) && !(isset($_GET["traite"]) ? $_GET["traite"] : '') == 1)
	echo '<th colspan="2">Statut</th>';
echo '<th>Infos</th>';
echo '<th>Action</th>';
echo '</tr>';
$key = 0;
while ($val_viewcommande = mysqli_fetch_array($sql_viewcommande_result))
{
  if ($key%2 == 0)
  ...
  $sql_membre = "SELECT membre.id_membre, membre.login, membre.nom, membre.prenom, membre.email, membre.tel, membre.aplan
                 ...
                 ;";
  $sql_membre_result = mysqli_query($link,$sql_membre);
  $val_membre = mysqli_fetch_array($sql_membre_result);
...
$urlModif = $_SERVER['PHP_SELF'].'?page=viewcommande';

if(isset($_GET["traite"]) && $_GET["traite"]==1)
	$urlModif = $_SERVER['PHP_SELF'].'?page=viewcommande&traite=1';

echo '<a href="'.$urlModif.'&startrow=' . ($startrow+30) . '"> Suivant </a>';

$prev = $startrow - 30;

//only print a "Previous" link if a "Next" was clicked
if ($prev >= 0)
    echo '<br><a href="'.$urlModif.'&startrow=' . ($prev) . '"><br> Précédent </a>';
?>
