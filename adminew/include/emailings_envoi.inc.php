...
if (isset($_POST['MAX']) && is_numeric($_POST['MAX']) && isset($_POST['ID']))
{
	...
	$req_emails = mysqli_query($link,$sql_emails) or die ("Erreur SQL : ".mysqli_error($link));
	$nbr_emails = mysqli_num_rows($req_emails);
	...
	while ($data_emails = mysqli_fetch_array($req_emails))
	{
		$now = "";
		...
		if ($mail->Send())
		{
			...
			$req_done = mysqli_query($link,$sql_done) or die ("Erreur SQL : ".mysqli_error($link));
		}
		else
		{
