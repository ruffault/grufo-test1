...
//Nombre de champs langue source langue cible ‡ traiter
$nblangue = 15;
...
if(checkcategorie($_SESSION["product_categorie"]))
  $error .= "CatÈgorie choisie invalide<br />";
if(checkreference($_SESSION["product_reference"]))
  $error .= "RÈfÈrence invalide<br />";
...
if(isset($_SESSION["product_prix_editeur"]) && $_SESSION["product_prix_editeur"] != '' && checkprix($_SESSION["product_prix_editeur"]))
  $error .= "Prix Èditeur invalide<br />";
...
if(isset($_SESSION["product_delai_reapprovisionnement"]) && $_SESSION["product_delai_reapprovisionnement"] != '' && checkdelai($_SESSION["product_delai_reapprovisionnement"]))
  $error .= "DÈlai de rÈapprovisionnement invalide<br />";
...
if(checkanneeparution($_SESSION["product_anneeparution"]))
  $error .= "AnnÈe de parution invalide<br />";
...
if ($_POST["product_submitaddproduct"] == "Ajouter" and $error == "")
{
  ...
  //recuperation de l'id du produit insÈrÈ
  $id_newproduct = mysqli_insert_id($link);

  //Insertion de l'id des langues sÈlÈctionnÈ et de l'id du produit dans la table langue_produit
  for ($d=1; $d < ($nblangue + 1) ;$d++)
  {
...
if ($_POST["product_submitaddproduct"] == "Enregistrer les modifications" and $error == "")
{
  ...
  //On supprime les anciennes langues associÈes au produit
  ...
  //On regarde si le produit est dÈja en vitrine
...
$_SESSION["product_carrousel"] = "";
  ...
  //on envoier sur une page confirmant la mise ” jour du produit
...
if ($_POST["product_submittradproduct"] == "traduire" and $error == "")
{
	...
	$nom_fr =  $_SESSION["product_name_fr"] = stripslashes($_SESSION["product_name_fr"]);
	$des_fr =   $_SESSION["product_description_fr"] = stripslashes($_SESSION["product_description_fr"]);
	$info_divers_fr = $_SESSION["product_divers_fr"] = stripslashes($_SESSION["product_divers_fr"]);
	...
	//On modifie les informations du produit
		$sql_tradproduct = "UPDATE `produit`
                     SET
                     nom_fr = '".addslashes($nom_fr)."',
                     nom_de = '".utf8_decode($nom_de)."',
                     nom_es = '".utf8_decode($nom_es)."',
                     nom_it = '".utf8_decode($nom_it)."',
                     nom_en = '".utf8_decode($nom_en)."',
                     info_divers_fr = '" .addslashes($info_divers_fr). "',
                     info_divers_de = '" .utf8_decode($info_divers_de). "',
                     info_divers_es = '" .utf8_decode($info_divers_es). "',
                     info_divers_it = '" .utf8_decode($info_divers_it). "',
                     info_divers_en = '" .utf8_decode($info_divers_en). "',
                     description_fr = '" .addslashes($des_fr). "',
                     description_de = '" .utf8_decode($des_de). "',
                     description_es = '" .utf8_decode($des_es). "',
                     description_it = '" .utf8_decode($des_it). "',
                     description_en = '" .utf8_decode($des_en). "'
                     WHERE `id_produit` = '" . addslashes($_GET["id_produit"]) . "' LIMIT 1
                    ;";
