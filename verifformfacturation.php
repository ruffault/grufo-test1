...
if ($_POST["facturation_precise"] == "1")
{
  ...
  if ($_POST["facturation_pays"] && !$_POST["facturation_cp"])
  {
    $sql_cp_oblig = "SELECT *
                     ...
                     AND abrev = '" . mysqli_real_escape_string($link,$_POST["facturation_pays"]) . "'
                     LIMIT 1;";
...
//S'il les champs sont correctement renseignés (!$erreur_facturation)
{
  ...
  $sql_facturation = "INSERT INTO facturation(`id_facturation`,`civilite`,
                      ...
                      VALUES(
                             '',
                             '". mysqli_real_escape_string($link,$_POST["facturation_civilite"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_nom"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_prenom"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_adresse1"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_adresse2"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_adresse3"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_ville"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_etat"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_cp"])."',
                             '". mysqli_real_escape_string($link,$_POST["facturation_pays"])."'
                            );";
  ...
  $sql_facturation = "UPDATE commande
                    SET id_facturation = '".mysqli_real_escape_string($link,$_SESSION["id_facturation"])."'
                    WHERE id_commande = '".mysqli_real_escape_string($link,$_SESSION["id_commande"])."'
                  ;";
  mysqli_query($link,$sql_facturation);
// on envoie sur la page de selection du mode de paiement
if ($_SESSION['total_ebook'] != 0 && $_SESSION['total_ebook'] == $_SESSION['total_produits'])
	$redirection = "Location: index.php?page=formmodepaiement";	
	else
  //sinon on  envoie sur la page de selection du mode de frais de port
