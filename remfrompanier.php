...
elseif ($_POST["typeaction"] == "decote")
{
	...
	$sql_future_result = mysqli_query($link,$sql_future);

	if (mysqli_num_rows($sql_future_result))
	{
		//oui, on récupére son id
		$val_future = mysqli_fetch_array($sql_future_result);
...
elseif ($_POST["typeaction"] == "remettrepanier")
{
  ...
  $sql_panier_future_result = mysqli_query($link,$sql_panier_future);

  while($val = mysqli_fetch_array($sql_panier_future_result))
  {
    $sql_move_panier = "UPDATE panier
