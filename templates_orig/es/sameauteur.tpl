  ...
  <div>
      ...
      {if $sameauteur[sameauteur].rabais != '0.00' && $sameauteur[sameauteur].prix ne '0.00'}
        {if $smarty.session.ht  == "ht"}
          <strong>{$sameauteur[sameauteur].prix_rabais_ht}€ sin IVA ({$sameauteur[sameauteur].prix_rabais}€ con IVA)</strong>
        {else}
          <strong>{$sameauteur[sameauteur].prix_rabais}€ con IVA ({$sameauteur[sameauteur].prix_rabais_ht}€ sin IVA)</strong>
        {/if}
      {else}
        {if $smarty.session.ht  == "ht" && $sameauteur[sameauteur].prix ne '0.00'}
      		<strong>{$sameauteur[sameauteur].prix_ht}€ sin IVA({$sameauteur[sameauteur].prix}€ con IVA)</strong>
        {elseif $sameauteur[sameauteur].prix ne '0.00'}
      		<strong>{$sameauteur[sameauteur].prix}€ con IVA({$sameauteur[sameauteur].prix_ht}€ sin IVA)</strong>
        {/if}
      {/if}
      </dd>
  	</dl>
