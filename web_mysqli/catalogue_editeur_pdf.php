...
if (isset($_GET['id_editeur']) && is_numeric($_GET['id_editeur']))
{
	...
	if (addslashes($_GET['id_editeur'])!=12 OR addslashes($_GET['id_editeur'])!=2){ $sql_add= "produit.id_editeur = '".addslashes($_GET['id_editeur'])."' 
					AND ";}
	else{
		$sql_add= "(editeur.nom like '%LMD%' OR editeur.nom like '%maison du dictionnaire%'
					) AND ";
	}
	$sql = "SELECT produit.nom_fr as nom, editeur.nom as nom_editeur, 
					...
					WHERE editeur.id_editeur = '".addslashes($_GET['id_editeur'])."'
					AND $sql_add produit.sommeil <> 1
