...
if(!$_SESSION["id_membre"])
{
  $sql_indicatif = "SELECT abrev,phonecode, nom_$applicationlang as nom
	...
	$sql_indicatif_result = mysqli_query($link,$sql_indicatif);
  $total = mysqli_num_rows($sql_indicatif_result);
  ...
  $x = 0;
  while($val_indicatif = mysqli_fetch_array($sql_indicatif_result))
  {
    $indicatif[$x] = array(
