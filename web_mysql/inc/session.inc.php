...
// si elle n'est pas valide on la detruit
if (mysql_num_rows($session_result) < 1 && isset($_SESSION["sessionid"]))
  session_unset();
...
$sql_iduser = "SELECT id_utilisateur, id_membre
               FROM utilisateur
               WHERE sessionid = '".$_SESSION["sessionid"]."';";
$sql_iduser_result = mysql_query($sql_iduser);
$val_iduser = mysql_fetch_array($sql_iduser_result);
...
$sql_selectlastcommande_result = mysql_query($sql_selectlastcommande);

//S'il ya une precedente commande non validee on recupere son id
if (mysql_num_rows($sql_selectlastcommande_result) > 0)
{
  $sql_selectlastcommande_result2 = mysql_query($sql_selectlastcommande);
  $val_lastcommande = mysql_fetch_array($sql_selectlastcommande_result2);
  ...
  $sql_selectlastcommande_result = mysql_query($sql_selectlastcommande);
  $val_lastcommande = mysql_fetch_array($sql_selectlastcommande_result);
  $id_commande = $val_lastcommande["id_commande"];
}
...
if ($_SESSION["id_membre"]<>'0')
{
  $allabout = "SELECT *
               ...
               ;";
  $allabout_result = mysql_query($allabout);

  $val_allabout = mysql_fetch_array($allabout_result);
