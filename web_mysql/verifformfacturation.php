...
if ($_POST["facturation_precise"] == "1")
{
  ...
  if ($_POST["facturation_pays"] && !$_POST["facturation_cp"])
  {
    $sql_cp_oblig = "SELECT *
                     ...
                     AND abrev = '" . mysql_real_escape_string($_POST["facturation_pays"]) . "'
                     LIMIT 1;";
...
//S'il les champs sont correctement renseignés (!$erreur_facturation)
{
  ...
  $sql_facturation = "INSERT INTO facturation(`id_facturation`,`civilite`,
                      ...
                      VALUES(
                             '',
                             '". mysql_real_escape_string($_POST["facturation_civilite"])."',
                             '". mysql_real_escape_string($_POST["facturation_nom"])."',
                             '". mysql_real_escape_string($_POST["facturation_prenom"])."',
                             '". mysql_real_escape_string($_POST["facturation_adresse1"])."',
                             '". mysql_real_escape_string($_POST["facturation_adresse2"])."',
                             '". mysql_real_escape_string($_POST["facturation_adresse3"])."',
                             '". mysql_real_escape_string($_POST["facturation_ville"])."',
                             '". mysql_real_escape_string($_POST["facturation_etat"])."',
                             '". mysql_real_escape_string($_POST["facturation_cp"])."',
                             '". mysql_real_escape_string($_POST["facturation_pays"])."'
                            );";
  mysql_query($sql_facturation);

  //on passe l'id de cette facturation en session
  $_SESSION["id_facturation"] = mysql_insert_id();

  //on lie cette facturation à la commande
  $sql_facturation = "UPDATE commande
                    SET id_facturation = '".mysql_real_escape_string($_SESSION["id_facturation"])."'
                    WHERE id_commande = '".mysql_real_escape_string($_SESSION["id_commande"])."'
                  ;";
  mysql_query($sql_facturation);
