...
use MoneticoDemoWebKit\Monetico\Request\CofidisPaymentInformations;
use MoneticoDemoWebKit\Monetico\Request\PaymentRequest;

/**
 *  Example of a payment request that prefills the COFIDIS form
 */
class CofidisPreFillPaymentExample implements IPaymentRequestExample
{
    ...
    public function __construct()
    {
         ...
         */
        // Following parameters allows you to prefill the COFIDIS form
        $cofidis = new CofidisPaymentInformations();
        $cofidis->setPrenomClient("John");
        $cofidis->setNomClient("Doe");
        $cofidis->setAdresseClient("18 rue de l'adresse");
        $cofidis->setCodePostalClient("75100");
        $paymentRequest->setCofidis($cofidis);

        // Uncomment this line to enable automatic redirection to the cofidis form
        // Do not forget to change the protocol if necessary
        //$paymentRequest->setProtocole(PaymentProtocol::COFIDIS_3X_CB);
    ...
    {
        return "This example shows how to pre-fill the COFIDIS form (require the COFIDIS option on your EPT).";
    }

    public function getName()
    {
        return "COFIDIS form pre filling (requires COFIDIS option on your EPT)";
    }
