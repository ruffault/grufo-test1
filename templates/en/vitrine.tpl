<div id='vithome'>
    ...
    {if $vitrine[vitrine].rabais != '0.00'}
      {if $smarty.session.ht  == "ht"}
        <strong>{$vitrine[vitrine].prix_rabais_ht} € excl. VAT ({$vitrine[vitrine].prix_rabais} € incl. VAT)</strong>
      {else}
        <strong>{$vitrine[vitrine].prix_rabais} € incl. VAT ({$vitrine[vitrine].prix_rabais_ht} € excl. VAT)</strong>
      {/if}
    {else}
      {if $smarty.session.ht  == "ht"}
    		<strong>{$vitrine[vitrine].prix_ht} € excl. VAT ({$vitrine[vitrine].prix} € incl. VAT)</strong>
      {else}
    		<strong>{$vitrine[vitrine].prix} € incl. VAT ({$vitrine[vitrine].prix_ht} € excl. VAT)</strong>
      {/if}
    {/if}
    		</div>
    			{if $vitrine[vitrine].prix != 0 && $vitrine[vitrine].sommeil != 1}
    				<form class="addpan"  method="get" action="{$urlsite}addtopanier.php?nbpage=1&amp;pagecourante=1&amp;nbresult=1">
      					<div>
        					<input type="image" value="Add to basket" src="{$urlsite}lang/{$applicationlang}/img/addpan.png"/>
