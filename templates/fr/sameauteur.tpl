  ...
  <div>
      ...
      {if $sameauteur[sameauteur].rabais != '0.00' && $sameauteur[sameauteur].prix ne '0.00'}
        {if $smarty.session.ht  == "ht"}
          <strong>{$sameauteur[sameauteur].prix_rabais_ht}€ HT ({$sameauteur[sameauteur].prix_rabais}€ TTC)</strong>
        {else}
          <strong>{$sameauteur[sameauteur].prix_rabais}€ TTC ({$sameauteur[sameauteur].prix_rabais_ht}€ HT)</strong>
        {/if}
      {else}
        {if $smarty.session.ht  == "ht" && $sameauteur[sameauteur].prix ne '0.00'}
      		<strong>{$sameauteur[sameauteur].prix_ht}€ HT({$sameauteur[sameauteur].prix}€ TTC)</strong>
        {elseif $sameauteur[sameauteur].prix ne '0.00'}
      		<strong>{$sameauteur[sameauteur].prix}€ TTC({$sameauteur[sameauteur].prix_ht}€ HT)</strong>
        {/if}
      {/if}
      </dd>
  	</dl>
