<div id='vithome'>
    ...
    {if $vitrine[vitrine].rabais != '0.00'}
      {if $smarty.session.ht  == "ht"}
        <strong>{$vitrine[vitrine].prix_rabais_ht} € sin IVA ({$vitrine[vitrine].prix_rabais} € con IVA)</strong>
      {else}
        <strong>{$vitrine[vitrine].prix_rabais} € con IVA ({$vitrine[vitrine].prix_rabais_ht} € sin IVA)</strong>
      {/if}
    {else}
      {if $smarty.session.ht  == "ht"}
    		<strong>{$vitrine[vitrine].prix_ht} € sin IVA ({$vitrine[vitrine].prix} € con IVA)</strong>
      {else}
    		<strong>{$vitrine[vitrine].prix} € con IVA ({$vitrine[vitrine].prix_ht} € sin IVA)</strong>
      {/if}
    {/if}
    		</div>
    			{if $vitrine[vitrine].prix != 0 && $vitrine[vitrine].sommeil != 1}
    				<form class="addpan"  method="get" action="{$urlsite}addtopanier.php?nbpage=1&amp;pagecourante=1&amp;nbresult=1">
      					<div>
        					<input type="image" value="Añadir a mi cesta de la compra" src="{$urlsite}lang/{$applicationlang}/img/addpan.png"/>
