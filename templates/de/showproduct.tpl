...
<div id="showproduct" >
	...
	{if ($showproduct.realname)}
		<p id="realname">Originaltitel : {$showproduct.realname|utf8_encode}</p>
  {/if}
    ...
    {/if}
      ...
      {if $smarty.session.ht  == "ht"}
        <span id="prixediteur">Verlagspreis: {$showproduct.prix_editeur_ht} € zzgl. MwSt</span><br/>
      {else}
        <span id="prixediteur">Verlagspreis : {$showproduct.prix_editeur} € einschl. MwSt</span><br/>
      {/if}
    {/if}
    ...
    {if $showproduct.prix ne '0.00'}
      {if $showproduct.rabais != '0.00'}
        {if $smarty.session.ht  == "ht"}
          <span id="prix">{$showproduct.prix_rabais_ht} € zzgl. MwSt ({$showproduct.prix_rabais}€ einschl. MwSt)</span>
        {else}
          <span id="prix">{$showproduct.prix_rabais} € einschl. MwSt ({$showproduct.prix_rabais_ht}€ zzgl. MwSt)</span>
        {/if}
      {else}
        {if $smarty.session.ht  == "ht"}
          <span id="prix">{$showproduct.prix_ht} € zzgl. MwSt ({$showproduct.prix}€ einschl. MwSt)</span>
        {else}
          <span id="prix">{$showproduct.prix} € einschl. MwSt ({$showproduct.prix_ht}€ zzgl. MwSt)</span>
        {/if}
      {/if}
    {/if}
    {if $showproduct.disponible == 0 && $showproduct.delai_reapprovisionnement && $showproduct.prix ne '0.00' && $showproduct.sommeil != 1}
      Artikel lieferbar binnen {$showproduct.delai_reapprovisionnement} Tagen<br />
    ...
    <form method="get" action="{$urlsite}addtopanier.php?nbpage=1&amp;pagecourante=1&amp;nbresult=1">
      <div>
        <input type="image" value="In den Warenkorb" src="{$urlsite}lang/{$applicationlang}/img/addcaddy_big.gif" />
    ...
    {/if}
		...
		<img src="{$urlsite}css/recommend_friend.png" alt="" />Buch einem/r Bekannten empfehlen]</a>
  </div>  
  ...
  <div id='info'>
    ...
    {if $showproduct.info_divers}
        <br>
      <p>Sonstige Informationen :{$showproduct.info_divers|utf8_encode}</p>
    {/if}
    {if $showproduct.pdfap}
      <h4><span class="deco">></span><a href="{$showproduct.pdfap}"> Download einen Überblick über das Buch</a></h4>
    {/if}
  </div>
  {/if}
				...
				<td>
					<ul>
						{if !$showproduct.tab_lang && $showproduct.langues}
							<li><strong>Sprachen : </strong>{$showproduct.langues|utf8_encode}</li>
						{elseif	!$showproduct.tab_lang.0.cible && $showproduct.tab_lang.0.source}
							<li><strong>Sprachen : </strong>{$showproduct.tab_lang.0.source|utf8_encode}</li>
						{/if}
						{if $showproduct.categorie}
							<li><strong>Kategorie: </strong><a href="{$urlsite}{$showproduct.id_categorie|categ_link:$showproduct.categorie}">{$showproduct.categorie|utf8_encode}</a></li>
						{/if}
						{if $showproduct.auteur}
							<li><strong>Autor: </strong>{$showproduct.auteur|utf8_encode}</li>
						{/if}
						{if $showproduct.nb_pages}
							<li><strong>Seitenzahl: </strong>{$showproduct.nb_pages}</li>
						{/if}
					</ul>
				</td>
				<td>
					<ul>
						{if $showproduct.nb_termes}
							<li><strong>Anzahl Einträge: </strong>{$showproduct.nb_termes}</li>
						{/if}
						{if $showproduct.index}
							<li><strong>Index vorhanden</strong></li>
						{/if}
						{if $showproduct.date_parution}
							<li><strong>Erscheinungsdatum: </strong>{$showproduct.date_parution}</li>
						{/if}
					</ul>
				</td>
				<td>
			      ...
			      {if $showproduct.reference}
							<li><strong>Bestell-Nr.: </strong>{$showproduct.reference}</li>
						{/if}
						{if $showproduct.type}
							<li><strong>Produktart: </strong>{$showproduct.type}</li>
						{/if}
						{if $showproduct.prix_editeur ne '0.00'}
							<li><strong>Verlagspreis:</strong>
							{if $smarty.session.ht  == "ht"}
								{$showproduct.prix_editeur_ht} € zzgl. MwSt 
							{else}
								{$showproduct.prix_editeur} € einschl. MwSt
							{/if}
							</li>
						{/if}
