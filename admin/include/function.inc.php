...
{
	if (!ereg("^[0-9]{1,8}$", $poid))
		return true;
}

//Test du délai de réapprovisionnement
function checkdelai($delai)
{
	if (!ereg("^[0-9]{1,5}$", $delai))
		return true;
}
...
function checkjourparution($jour)
{
  //test s'il est numerique, s'il fait bien 2 chiffres et s'il est compris entre 0 et 13
	if (!ereg("^([0-9]{1,2})$", $jour) or $jour > 31 or $jour < 1)
		return true;
}
...
function checkmoisparution($mois)
{
  //test s'il est numerique, s'il fait bien 2 chiffres et s'il est compris entre 0 et 32
	if (!ereg("^([0-9]{1,2})$", $mois) or $mois > 12 or $mois < 1)
		return true;
}

// Test l'année de parution
function checkanneeparution($annee)
{
  //test si elle est numerique, et si elle fait bien de 1 à 4 chiffres
	if (!ereg("^([0-9]{1,4})$", $annee))
		return true;
}
...
function poid_commande($id_commande)
{
  ...
  while($val = mysql_fetch_array($sql_poid_result))
  {
	...
	mysql_free_result($sql_poid_result);
...
function frais_port($poid, $id_livraison)
{
  ...
  //On determine la tranche Ã  appliquer suivant le poid
  if ($poid <= 250)
    $limite = 250;
  elseif ($poid <= 500)
    $limite = 500;
...
function infos_frais_port_for_admin($id_frais_port,$id_commande)
{
  ...
  $res_nom_frais_port = mysql_query($sql_nom_frais_port);
  $val_nom_frais_port = mysql_fetch_array($res_nom_frais_port);
  ...
  $res_prix_frais_port = mysql_query($sql_prix_frais_port);
  $val_prix_frais_port = mysql_fetch_array($res_prix_frais_port);
...
function date_fr($timestamp=0)
{
	...
	$tabmois = array('0', 'janvier', 'février', 'mars', 'avril',
...
{
	$url = ereg_replace('^.*(\?p=|&p=|\?va=|&va=)','',$url);
	$url = ereg_replace('&.*$','',$url);
...
function delivery_info($id_delivery)
{
	...
	if (isset($res))
	{
		$delivery = mysql_fetch_array($res);
...
function statut2txt($num)
{
	switch ($num)
	{
		case '1':
			...
			break;
		case '2':
			$txt = 'Commande refusée';
			break;
		case '3':
			$txt = 'Commande réglée';
			break;
		case '4':
			$txt = 'Commande accéptée';
			break;
		case '5':
			$txt = 'Commande en cours de traitement';
			break;
		case '6':
			$txt = 'Commande expédiée';
			break;
		case '8':
			$txt = 'Commande masquée';
			break;
		case '9':
...
function Is_ValidePrixPort($prix_port)
{
	...
	if (ereg('^[0-9]{1,3}(\.[0-9]{1,2})?$', $prix_port))
	{
		$ReturnVar = 1;
	}
	return $ReturnVar;
...
function FormateDateTimeFR($DateUStoFormat,$separateur)
{
	...
	if ($heure)
	{
		return($jour.$separateur.$mois.$separateur.$annee." à ".$heure);
	}
	else
	{
		return($jour.$separateur.$mois.$separateur.$annee);	
...
{
	$returnVar = 0;
	if ( ereg( "(^[0-9]{4})-([0-1]{1}[0-9]{1})-([0-3]{1}[0-9]{1}$)", $DateUS) )
	{
		$returnVar = 1;
	}
	return $returnVar;
 ...
 *
 * @param string $emailToVerify email à vérifier
 *
...
{
	$returnVar = 0;
	if (eregi("^[0-9a-z]([-_.]?[0-9a-z])*@[0-9a-z]([-.]?[0-9a-z])*\\.[a-z]{2,3}$", $emailToVerify)) 
	{
		$returnVar = 1;
	}
	return $returnVar;
...
/**
 * Fonction récupérant la liste des fichiers contenus dans le dossier des pieces jointes d'emailings
 * les plaçant dans un tableau où ils sont triés par ordre alphabétique.
 * Ne prend que les fichier ayant comme format de nom ("[ID_EMAILING]_[nomFichierSansEspaces]_.[extension]")
 * 
 * @return array $array_file tableau contenant les fichiers classés par ordre alphabétique.
 */
function getArrayEmailingsPJFile($ID_EMAILING,$PATH_PJ)
{
	$array_file = array();
	// si le dossier censé contenir les pieces jointes n'existe pas, on renvoie NULL.
	...
	{		
		if ( (filetype("".$PATH_PJ."/".$file."") == "file") && (ereg("^".$ID_EMAILING."_+([a-zA-Z0-9_])+\.([a-zA-Z]{3})$",$file)) )
		{
...
function NettoyerChaineParUnderscores($chaine)
{
	...
	while($num_caractere < strlen($chaine))
	{
		...
		if(!ereg("^([a-z0-9_{1}])$", $caractere_actuel))
		{
			$nouvelle_chaine.= "_";
		}
		else
...
/**
 * Fonction récupérant une date au format FR à partir :
...
function getTotalEmailNumberOfEmailing($ID_EMAILING)
{
  ...
  $result = mysql_query($query);
  $num_destinataires = mysql_numrows($result);
