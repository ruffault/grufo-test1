...
if (isset($_POST["action"]) && $_POST["action"] == "save")
{
	...
	if (!$nbr_erreurs)
	{
		...
		if (isset($_POST["ID_MSG"]))
		{
			...
			$res_save_edited_message = mysql_query($sql_save_edited_message);
		}
		else
		{
			...
			$res_save_message = mysql_query($sql_save_message);
		}
	}
}
...
if (isset($_GET["action"]) && $_GET["action"] == "delete" && isset($_GET["ID_MSG"]) && is_numeric($_GET["ID_MSG"]))
{
	...
	$res_delete_messages = mysql_query($sql_delete_messages);
}
...
if (isset($_GET["action"]) && $_GET["action"] == "edit" && isset($_GET["ID_MSG"]) && is_numeric($_GET["ID_MSG"]))
{
	...
	$res_get_message = mysql_query($sql_get_message);
	while ($val_get_message = mysql_fetch_array($res_get_message))
	{
...
$res_liste_messages = mysql_query($sql_liste_messages);
?>
<div id="formlistemessages">
<table>
<?php
while ($val_liste_messages = mysql_fetch_array($res_liste_messages))
{
	?>
	<tr class="titre">
