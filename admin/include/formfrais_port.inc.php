...
if (isset($_POST["do"]) && $_POST["do"] == "Enregistrer" && isset($_POST["PAYS"]) && trim($_POST["PAYS"]) && isset($_POST["id_frais_port"]) && is_numeric($_POST["id_frais_port"]))
{
	...
	{
		if (ereg('^[0-9]{3,5}$', $cle) && !Is_ValidePrixPort($valeur))
		{
...
<table border="0">
	...
	<tr>
		<td>
			...
			GROUP BY frais_port.pays
			ORDER BY frais_port.pays ASC";
			$req_liste_pays = mysql_query($sql_liste_pays)  or die ("Erreur SQL : ".mysql_error());
			while ($data_liste_pays = mysql_fetch_array($req_liste_pays))
			{
				?>
