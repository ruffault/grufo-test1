...
function insert_referermonth2($idmonth,$idreferer,$counter)
{
...
function selectmonth2()
{
	...
	{
		$date = $val['year'] . '-' . date2letter2($val['month']);
...
function all_referer2()
{
	$i = 0;
	
	$selectedmonth = selectmonth2();
	...
	while($val = mysql_fetch_assoc($res))
	{
		...
		if($plop != '')
		{
			...
			{
				$id = referer_id2($matches[0]);
				if (isset($id))
				{
					$id_month = id_date2($val['year'], $val['month']);
...
$sql = 'DELETE FROM statreferer WHERE id_month = "'
	.id_date2(date('Y'), date('m')) . '";';
mysql_query($sql);

$tabreferer = all_referer2();
...
foreach($tabcount as $idreferer => $value)
{
	...
	{
		insert_referermonth2($month, $idreferer, $count);
	}
}
