...
function checklogin($login) {
	if (!preg_match( "/^([a-zA-Z0-9_\-\.]{2,20})$/", $login))
		return true;
}
...
function checkzip($zip) {
	if (!preg_match("/^([a-zA-z0-9]{2,7})$/", $zip))
		return true;
}
...
{
	if (!preg_match("/^([0-9]{1,20})$/", $telephone))
		return true;
}
...
//Indique si le panier contient un livre d'un editeur maison
function edi_maison($id_commande)
{
	global $link;
$panier_maison=0;
$sql_maison= "SELECT editeur.maison
              FROM editeur, panier, commande,produit
              WHERE panier.id_commande = commande.id_commande
              AND commande.id_commande = '".$id_commande."'
              AND panier.id_produit = produit.id_produit
              AND produit.id_editeur = editeur.id_editeur
              AND editeur.maison
             ;";
$res_maison= mysqli_query($link,$sql_maison);
$panier_maison = mysqli_num_rows($res_maison);
return ($panier_maison <>0); 
}
//permet de trouver la borne applicable aux frais de prot en fonction du poids
function limite($poids)
{
$tab = array(250,500,750,1000,2000,3000,4000,5000,6000,7000,8000,9000,10000,11000,12000,13000,14000,15000,16000,17000,18000,19000,20000,25000,30000);
foreach ($tab as $borne){
$res = $borne;
if ($poids <= $borne) break;
}
return $res;
}


//Nouvelle fonction prenant en compte la restructuration du moteur de calcul basé sur la table frais_port_new
//GR le 30 octobre 2020
function frais_port_new($poids,$pays,$mode_calcul)
{
	global $link;
	$frais_port = NULL;
	switch ($mode_calcul) {
	case 1:	
// correspond a panier contenant un livre maison
// à ce stade est géré dans le cas standard
//
//
	default: //on est dans le cas standard géré avec une selection dans la table pilotée par le mode_calcul 	
// on selectionne pour le pays et le mode de calcul donné, les couples <mode, prix> correspondant au poids de la commande
	$sql = "SELECT mode, MIN(prix) as prix
		FROM frais_port_new
		WHERE mode = " . $mode_calcul . "
		AND poids_tarif >= " . $poids ."
		AND pays = '" . $pays ."'
		GROUP BY mode
		;";  
	...
	while ($val = mysqli_fetch_array($res))
	{
		...
		$frais_port[$i]['prix'] = twodecimal($val['prix']);
		$i++;
	}
...
return $frais_port ;
} // fin de la fonction frais_port_new


//Prix des frais de port quand panier maison (a priori limité à la France
function frais_port_maison($poid, $id_livraison)
{
	global $link;
  ...
  $sql = "SELECT mode, `" . $limite . "` FROM frais_port
	...
	//On propose le colissimo gratuit quand le poids ne permet pas le courrier ordinaire pour la France
        if ($poid > 3000 && $pays == "FR")
        {
                $frais_port[0]['mode'] = 'colissimo.fr';
		$frais_port[0]['prix'] = '0.01';
        }
        ...
        else  //on va chercher les modes possibles avec leur cout et rendre gratuit en courrier ordinaire
        {
		$sql.=";";
	}
...
while ($val = mysqli_fetch_array($res))
  {
    $frais_port[$i]['mode'] = $val['mode'];
                ...
                {
                    $frais_port[$i]['prix'] = '0.01';
		// dans ce cas la solution courrier ordinaire est (pseudo) gratuite
               } 
                else if ($frais_port[$i]['mode'] == 'courrier_ordinaire')
		{
                    $frais_port[$i]['prix'] = '0.01';
		}
                
		// et les autres modes sont proposés au tarif normal
	        else
		{
...
//fin de la function frais_port_maison
...
function tab_lang($id)
{
	...
	$query = "SELECT nom_$applicationlang as nom
						...
						;";
	$res_src = mysqli_query($link,$query);
	while($val_src = mysqli_fetch_array($res_src))
...
function give_basket($commandid)
{
	...
	$sql = 'SELECT panier.id_panier,
									...
									produit.id_type as type_produit
...
function basket_summary($basket)
{
	...
	$summary['total_ttc'] = 0;
	$summary['total_ebook'] = 0;
	$summary['total_produits'] = 0;
	...
	if(is_array($basket)){
		foreach ($basket as $key => $product)
		{
			...
			//j'ajoute le calcul du nombre d'ebooks et du nombre total de produits dans le panier- GR MAI 2020
			$summary['total_produits']++;
			if ($product['type_produit'] == 3) $summary['total_ebook']++; 

		}
	}
	$summary['total_ht'] = twodecimal(isset($summary['total_ht']) ? $summary['total_ht'] : '');
	$summary['total_taxe'] = twodecimal(isset($summary['total_taxe']) ? $summary['total_taxe'] : '');
	$summary['total_ttc'] = twodecimal(isset($summary['total_ttc']) ? $summary['total_ttc'] : '');
...
function order_address_ebook($commandid, $applicationlang)
{
	global $link;
		$sql = "SELECT
							membre.id_membre as membre_id_membre,
...
function give_product($id_product)
{
	...
	$res = mysqli_query($link,$sql);
	$val = mysqli_fetch_assoc($res);
//echo "cest la:". $val['realname'];
	$date_piece = explode('-', $val["date_parution"]);
	$date = (isset($date_piece[2])? $date_piece[2] :'') . "/" . (isset($date_piece[1]) ? $date_piece[1] : '') . "/" . $date_piece[0];
	...
	$val['prix_ht'] = (isset($val['prix']) ? $val['prix'] : '');
	$val['prix'] = (isset($val['prix']) ? $val['prix'] : '') + twodecimal(round_up(((isset($val['prix']) ? $val['prix'] : '') * (isset($val['tva']) ? $val['tva'] : '') / 1000), 2));

	$val['rabais_ht'] = twodecimal(round_up(($val['prix_ht'] * (isset($val['rabais']) ? $val['rabais'] : '') / 100), 2));
	$val['prix_rabais_ht'] = twodecimal($val['prix_ht'] - $val['rabais_ht']);
	$val['prix_taxes'] = twodecimal(round_up(($val['prix_rabais_ht'] * (isset($val['tva']) ? $val['tva'] : '') / 1000), 2)) ;
	$val['prix_rabais'] = twodecimal($val['prix_rabais_ht'] + $val['prix_taxes']);
/* je commente toute cette partie la qui parait, curieusement, ne servir à rien sinon créer des notice
	$basket[$i]['prix_unitaire_ttc'] = twodecimal($val['prix_unitaire_ht'] + $basket[$i]['prix_taxes']);
	$basket[$i]['prix_unitaire_ttc_rabais'] = twodecimal($basket[$i]['prix_unitaire_ht_rabais'] + $basket[$i]['prix_taxes']);
	$basket[$i]['prix_rabais_for_all'] = twodecimal($basket[$i]['prix_rabais'] * $basket[$i]['quantite']);
	$basket[$i]['prix_unitaire_rabais_ht_for_all'] = twodecimal($basket[$i]['prix_unitaire_ht_rabais'] * $basket[$i]['quantite']);
	$basket[$i]['prix_taxes_for_all'] = twodecimal($basket[$i]['prix_taxes'] * $basket[$i]['quantite']);
 */
	$product = array("id_produit" => (isset($val["id_produit"]) ? $val["id_produit"] : '') ,
								"nom" => (isset($val["nom"]) ? $val["nom"] : '') ,
								"realname" => (isset($realname) ? $realname : '') ,
								"reference" => (isset($val["reference"]) ? $val["reference"] : '') ,
								"isbn" => (isset($val["isbn"]) ? $val["isbn"] : '') ,
								"id_categorie" => (isset($val['id_categorie']) ? $val['id_categorie'] : '') ,
								"categorie" => (isset($val["categorie"]) ? $val["categorie"] : '') ,
								"id_auteur" => (isset($val['id_auteur']) ? $val['id_auteur'] : '') ,
								"auteur" => (isset($val["auteur"]) ? $val["auteur"] : '') ,
								"thematic" => (isset($val["thematic"]) ? $val["thematic"] : '') ,
								"meta" => (isset($val["meta"]) ? $val["meta"] : '') ,
								"prix" => (isset($val["prix"]) ? $val["prix"] : '') ,
								"prix_ht" => (isset($val["prix_ht"]) ? $val["prix_ht"] : '') ,
								"prix_rabais" => (isset($val["prix_rabais"]) ? $val["prix_rabais"] : '') ,
		"prix_rabais_ht" => (isset($val["prix_rabais_ht"]) ? $val["prix_rabais_ht"] : '') ,
								"prix_editeur" => (isset($val["prix_editeur"]) ? $val["prix_editeur"] : '') ,
								"prix_editeur_ht" => (isset($val["prix_editeur"]) ? $val["prix_editeur"] : '') ,
								"langues" => (isset($val["langues"]) ? $val["langues"] : '') ,
								"tab_lang" => (isset($tab_lang) ? $tab_lang : '') ,
								"rabais" => (isset($val["rabais"]) ? $val["rabais"] : '') ,
								"delai_reapprovisionnement" => (isset($val["delai_reapprovisionnement"]) ? $val["delai_reapprovisionnement"] : '') ,
								"disponible" => (isset($val["disponible"]) ? $val["disponible"] : '') ,
								"type" => (isset($val["type"]) ? $val["type"] : '') ,
								"tva" => (isset($val["tva"]) ? $val["tva"] : '') ,
								"nb_pages" => (isset($val["nb_pages"]) ? $val["nb_pages"] : '') ,
								"nb_termes" =>  (isset($val["nb_termes"]) ? $val["nb_termes"] : '') ,
								"url" =>  (isset($val["url"]) ? $val["url"] : '') ,
								"pdfap" =>  (isset($val["pdfap"]) ? $val["pdfap"] : '') ,
								"index" =>  (isset($val["ind"]) ? $val["ind"] : '') ,
								"description" =>  (isset($val["description"]) ? $val["description"] : '') ,
								"info_divers" =>  (isset($val["info_divers"]) ? $val["info_divers"] : '') ,
								"date_parution" =>  (isset($date) ? $date : '') ,
								"score" => 'note' . (isset($val["score"]) ? $val["score"] : '') ,
								"img_type" => img_type((isset($val["id_produit"]) ? $val["id_produit"] : '') ),
								"sommeil" =>  (isset($val["sommeil"]) ? $val["sommeil"] : '')
								);
	//echo '<pre>' . print_r($product, 1) . '</pre>';
	return $product;
}
...
//création d'une fonction pour récupérer les données de la ebookthèque d'un membre donné

function give_ebooktheque($id_membre)
{
	global $link;
	global $applicationlang;

	$sql = "SELECT commande.code,commande.date_creation,commande.statut,panier.id_produit,panier.prix_unitaire,panier.quantite,panier.taxes,membre.id_membre,
		produit.id_produit, produit.id_type,produit.reference,produit.realname,produit.description_$applicationlang as nom,produit.image
		FROM commande,panier,membre,utilisateur,produit
		WHERE membre.id_membre = '" . $id_membre . "'
		AND (commande.statut = 3  or commande.statut = 6) AND produit.id_type = 3
		AND commande.id_commande = panier.id_commande AND commande.id_utilisateur = utilisateur.id_utilisateur
		AND utilisateur.id_membre = membre.id_membre AND panier.id_produit = produit.id_produit;";
...
function lit_ebook($id_produit) 
{
	$file = "http://dicotest.grufo.ovh/home/gerard/ebook_" . $id_produit;
	if (file_exists($file)) {
