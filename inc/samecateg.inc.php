...
$smarty->assign("samecateg", (isset($samecateg) ? $samecateg : '') );
...
$sqlpage = "SELECT count(*) as total
						...
						OR categorie.id_categorie_parent = '" . $_GET["idcat"] . "');";
$res = mysqli_query($link,$sqlpage);
$valpage = mysqli_fetch_array($res);
...
if ($allpage > 1)
{
	...
	if (($curpage - $nb_lienpage) > 0)
	{
		...
		$borne_inf = 1;
//j'ai modifié ici $nbpage en $nbcatpage puisqu'il n'y avait aucune reference a $nbpage
	if (($nbcatpage - $curpage) > 0) 
	...
	for ($i=$borne_inf; $i <= $borne_sup; $i++)
	{
		...
		{
			$lunk[$j] = array('num' => " <a href='catalogue-"
			. $_GET["idcat"]
			. "-"
			.((($i-1) * $nbcatpage)+1)
			. "-"
